...
export function parseStandardURIString(str) {
  ...
  let regex;
  let match; // XXX A URI string as defined by RFC 3986 does not contain any whitespace.
  ...
  if (match) {
    let authority = match[1].substring(
    /* // */
    2);
...
export function urlObjectToString(o) {
  ...
  const url = parseStandardURIString(_fixURIStringScheme(tmp)); // protocol
  ...
  if (!url.host) {
    ...
    if (domain && o.appLinkScheme) {
      const {
        ...
        port
      } = parseStandardURIString( // XXX The value of domain in supposed to be host/hostname
      // and, optionally, pathname. Make sure it is not taken for
      // a pathname only.
      _fixURIStringScheme(`${o.appLinkScheme}//${domain}`)); // authority
